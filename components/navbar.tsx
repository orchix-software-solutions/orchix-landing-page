...
export default function Navbar({ showAnnouncementBar = true }: { showAnnouncementBar?: boolean }) {
  ...
  return (
    <nav
      className={`fixed left-0 w-full px-[3rem] py-5 flex items-center justify-between backdrop-blur-[20px] bg-[rgba(10,10,10,0.6)] border-b border-[rgba(255,255,255,0.08)] transition-all duration-400 max-[1100px]:px-6 max-[1100px]:py-4 ${mobileOpen ? "z-[210]" : "z-[100]"} ${showAnnouncementBar ? "top-9 max-sm:top-[2.75rem]" : "top-0"}`}
      dir={isRTL ? "rtl" : "ltr"}
    >
      {/* Logo */}
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 top-0 z-[1] bg-[rgba(0,0,0,0.6)] backdrop-blur-sm transition-opacity duration-300 hidden max-[600px]:block ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        onClick={() => setMobileOpen(false)}
      />

      {/* Mobile Menu Panel */}
      <div
        className={`fixed top-0 ${isRTL ? "left-0" : "right-0"} z-[2] h-full w-[80vw] max-w-[320px] bg-[rgba(12,12,12,0.98)] backdrop-blur-[30px] border-l border-[rgba(255,255,255,0.08)] shadow-[-10px_0_40px_rgba(0,0,0,0.5)] transition-transform duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] hidden max-[600px]:block ${mobileOpen ? "translate-x-0" : (isRTL ? "-translate-x-full" : "translate-x-full")}`}
        dir={isRTL ? "rtl" : "ltr"}
      >
