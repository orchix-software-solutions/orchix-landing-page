...
export default function UrgencyBar({ onDismiss }: UrgencyBarProps) {
  ...
  return (
    <div
      data-urgency-bar
      dir={isRTL ? "rtl" : "ltr"}
      className="fixed top-0 left-0 w-full z-[200] bg-accent flex items-center justify-center gap-3 pl-4 pr-10 py-[0.55rem] text-[0.8rem] font-semibold tracking-[0.02em] text-white animate-fade-in-down group max-sm:text-[0.65rem] max-sm:gap-1.5 max-sm:py-1.5 max-sm:pl-3 max-sm:pr-9 max-[400px]:text-[0.6rem]"
    >
